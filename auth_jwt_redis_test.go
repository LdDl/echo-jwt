...
func TestEchoJWTMiddleware_RedisStore_Integration(t *testing.T) {
	...
	e.POST("/refresh", middleware.RefreshHandler)
	...
	t.Run("LoginAndRefreshWithRedis", func(t *testing.T) {
		testLoginAndRefreshFlow(t, e)
	})

	t.Run("TokenPersistenceAcrossRequests", func(t *testing.T) {
		testTokenPersistenceAcrossRequests(t, e)
	})
...
func TestEchoJWTMiddleware_RedisStoreFallback(t *testing.T) {
	// Create middleware with invalid Redis configuration (should fallback to memory)
	middleware := &EchoJWTMiddleware{
		Realm:         "test zone",
		Key:           []byte("secret key"),
		Timeout:       time.Hour,
		MaxRefresh:    time.Hour * 24,
		IdentityKey:   "id",
		Authenticator: redisTestAuthenticator,
		PayloadFunc:   redisTestPayloadFunc,
...
func TestEchoJWTMiddleware_FunctionalOptions(t *testing.T) {
	host, port := setupRedisContainerForJWT(t)
...
func createTestMiddleware(redisAddr string) *EchoJWTMiddleware {
	middleware := &EchoJWTMiddleware{
		Realm:         "test zone",
		Key:           []byte("secret key"),
		Timeout:       time.Hour,
...
func redisTestAuthenticator(c *echo.Context) (any, error) {
...
func redisTestPayloadFunc(data any) gojwt.MapClaims {
...
func testLoginAndRefreshFlow(t *testing.T, e *echo.Echo) {
	// Test login
...
func testTokenPersistenceAcrossRequests(t *testing.T, e *echo.Echo) {
...
func testRedisStoreOperations(t *testing.T, middleware *EchoJWTMiddleware) {
