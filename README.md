...
- 🔒 Simple JWT authentication for Echo
...
func registerRoute(r *echo.Echo, handle *jwt.EchoJWTMiddleware) {
  // Public routes
  r.POST("/login", handle.LoginHandler)
  r.POST("/refresh", handle.RefreshHandler) // RFC 6749 compliant refresh endpoint

  // e.RouteNotFound("/*", handle.MiddlewareFunc()(handleNoRoute()))
...
func handleNoRoute() echo.HandlerFunc {
  return func(c *echo.Context) error {
    return c.JSON(404, map[string]interface{}{"code": "PAGE_NOT_FOUND", "message": "Page not found"})
  }
}
...
The `EchoJWTMiddleware` struct provides the following configuration options:

| Option                 | Type                                             | Required | Default                  | Description                                                                                           |
| ---------------------- | ------------------------------------------------ | -------- | ------------------------ | ----------------------------------------------------------------------------------------------------- |
| Realm                  | `string`                                         | No       | `"echo jwt"`              | Realm name to display to the user.                                                                    |
...
| Authenticator          | `func(c *echo.Context) (any, error)`              | Yes      | -                        | Callback to authenticate the user. Returns user data.                                                 |
| Authorizer             | `func(c *echo.Context, data any) bool`            | No       | `true`                   | Callback to authorize the authenticated user.                                                         |
| PayloadFunc            | `func(data any) jwt.MapClaims`                   | No       | -                        | Callback to add additional payload data to the token.                                                 |
| Unauthorized           | `func(c *echo.Context, code int, message string)` | No       | -                        | Callback for unauthorized requests.                                                                   |
| LoginResponse          | `func(c *echo.Context, token *core.Token)`        | No       | -                        | Callback for successful login response.                                                               |
| LogoutResponse         | `func(c *echo.Context)`                           | No       | -                        | Callback for successful logout response.                                                              |
| RefreshResponse        | `func(c *echo.Context, token *core.Token)`        | No       | -                        | Callback for successful refresh response.                                                             |
| IdentityHandler        | `func(*echo.Context) any`                         | No       | -                        | Callback to retrieve identity from claims.                                                            |
...
```go
authMiddleware, err := jwt.New(&jwt.EchoJWTMiddleware{
    Realm:      "your realm",
...
func createMultiProviderAuthMiddleware() (*jwt.EchoJWTMiddleware, error) {
    // Your own JWT secret
    ...
    return jwt.New(&jwt.EchoJWTMiddleware{
        ...
        IdentityHandler: func(c *echo.Context) interface{} {
            claims := jwt.ExtractClaims(c)
        ...
        Authorizer: func(c *echo.Context, data interface{}) bool {
            claims := jwt.ExtractClaims(c)
            issuer, _ := claims["iss"].(string)
...
// Azure AD specific authorization
func authorizeAzureADUser(claims jwt.MapClaims, c *echo.Context) bool {
...
// Your own token authorization
func authorizeOwnUser(claims jwt.MapClaims, c *echo.Context) bool {
...
func MultiAuthMiddleware(
    ownAuth *jwt.EchoJWTMiddleware,
    externalAuth *jwt.EchoJWTMiddleware,
) echo.MiddlewareFunc {
    return func(next echo.HandlerFunc) echo.HandlerFunc {
        return func(c *echo.Context) error {
            // Try own authentication first
            handler := ownAuth.MiddlewareFunc()(func(c *echo.Context) error { return nil })
            handler(c)

            // Check if authentication succeeded
            if c.Get("JWT_PAYLOAD") != nil {
                return next(c)
            }

            // Try external provider
            handler = externalAuth.MiddlewareFunc()(func(c *echo.Context) error { return nil })
            handler(c)
...
- **KeyPrefix**: Prefix for all Redis keys (default: `"echo-jwt:"`)
...
```go
func(c *echo.Context, data any) bool
```
...
```go
func authorizeHandler() func(c *echo.Context, data any) bool {
    return func(c *echo.Context, data any) bool {
        if v, ok := data.(*User); ok && v.UserName == "admin" {
...
adminMiddleware, _ := jwt.New(&jwt.EchoJWTMiddleware{
    // ... other config
...
userMiddleware, _ := jwt.New(&jwt.EchoJWTMiddleware{
    // ... other config
    Authorizer: func(c *echo.Context, data any) bool {
        if user, ok := data.(*User); ok {
...
adminRoutes.Use(adminMiddleware.MiddlewareFunc())
userRoutes := r.Group("/user")
userRoutes.Use(userMiddleware.MiddlewareFunc())
```
...
This function is called after logout processing is complete. It should return the appropriate HTTP response to indicate logout success or failure. Since logout doesn't generate new tokens, this function only receives the Echo context. You can access JWT claims and user identity via `jwt.ExtractClaims(c)` and `c.Get(identityKey)` for logging or auditing purposes.

Signature: `func(c *echo.Context)`
